...
class Analyze:
    def __init__(self):
        self.sum_list = ['=SUM(B2:B7)', '=SUM(C2:C7)', '=SUM(D2:D7)', '=SUM(E2:E7)', \
                    '=SUM(F2:F7)', '=SUM(G2:G7)', '=SUM(H2:H7)', '=SUM(I2:I7)', \
                    '=SUM(J2:J7)', '=SUM(K2:K7)', '=SUM(L2:L7)', '=SUM(M2:M7)', \
                    '=SUM(N2:N7)', '=SUM(O2:O7)', '.', '=SUM(Q2:Q7)', '=SUM(R2:R7)']
        self.col_list = ['食費', '生活雑貨', '住居費', '交通費', '通信費', '光熱費',\
                    '教育教養費', '娯楽費', '被服費', '医療費', 'ギフト', '交際費', 'その他', '合計',\
                    '.', 'るな', 'ともこ']
        self.index_list = ['共通', 'ともこ', 'るな', 'ゆきまる', 'ハム', 'その他', '合計']

    def categorize_for_person(self, row):
        if row[6] == '共通':
...
def button2_clicked():
    incsv = file1.get()
    analyze = Analyze()
    outxlsx = analyze.analyze_data(incsv)
